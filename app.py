...
// Action via URL param (navigate parent)
function nav(params){{
  var url=new URL(window.parent.location.href);
  for(var k in params) url.searchParams.set(k,params[k]);
  window.parent.location.href=url.toString();
}}
function sbAct(a){{nav({{_sb:a,_sbt:Date.now().toString()}});}}
function doSend(){{
  var i=document.getElementById('inp'),txt=i?i.value.trim():'';
  if(!txt) return;
  i.value='';
  nav({{_cm:txt,_ct:Date.now().toString()}});
}}
function onKey(e){{if(e.key==='Enter'&&!e.shiftKey){{e.preventDefault();doSend();}}}}
function sendMsg(t){{nav({{_cm:t,_ct:Date.now().toString()}});}}
function doAttach(){{
  var f=document.createElement('input');f.type='file';
  f.accept='.pdf,.txt,.png,.jpg,.jpeg,.docx,.csv';
  f.onchange=function(){{if(f.files&&f.files[0]) nav({{_af:f.files[0].name,_at:Date.now().toString()}});}};
  f.click();
}}
function doMic(){{nav({{_mic:Date.now().toString()}});}}
function clearChip(){{
  var url=new URL(window.parent.location.href);
  url.searchParams.delete('_af');
  window.parent.location.href=url.toString();
}}
</script>
</body>
    ...
    # ── Render via components.html (this is the CORRECT way — no markdown injection) ──
    components.html(CHAT_HTML, height=760, scrolling=False)
