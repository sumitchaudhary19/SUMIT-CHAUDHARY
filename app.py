...
# ==========================================
# 3. CSS (UI & Static Dark Grey Tabs)
# ==========================================
st.markdown(f"""
    <style>
    ...
    /* --- DARK GREY TABS (Sub-tabs & Subject List) --- */
    /* Targetting buttons with 'subject_tab' help to apply dark grey */
...
with st.sidebar:
    ...
    if st.session_state.show_acad_menu:
        # Academics sub-tabs set to dark grey using help="subject_tab"
        if st.button("Syllabus", use_container_width=True, help="subject_tab"): 
            open_syllabus_list()
        if st.button("Notes", use_container_width=True, help="subject_tab"): 
            st.toast("Notes coming soon!")
        if st.button("PYQs", use_container_width=True, help="subject_tab"): 
