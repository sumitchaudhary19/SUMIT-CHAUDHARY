...
# ║                                                                              ║
# ║  ARCHITECTURE:                                                               ║
# ║  The entire chat UI (navbar, messages, input bar) is rendered as a single   ║
# ║  self-contained HTML document via st.components.v1.html.                    ║
# ║  This is the ONLY way to achieve pixel-perfect fixed-position elements,     ║
# ║  inline icon layout, and the shift effect inside Streamlit without           ║
# ║  fighting its render pipeline.                                               ║
# ║                                                                              ║
# ║  Communication flow:                                                         ║
# ║    HTML → Python : window.parent.postMessage({type, payload}, "*")          ║
# ║    Python renders messages back into the HTML via template injection         ║
# ║                                                                              ║
...
_DEFAULTS: dict = {
    ...
    "planner_overrides":  {},
    # Chat action routing (set by HTML component via query_params workaround)
    "chat_action":        None,
    "chat_input_text":    "",
}
...
def generate_ai_response(last: str) -> str:
    ...
    if any(w in lower for w in ["schedule","class","next","today","timetable"]):
        if st.session_state.schedule_loaded:
            ...
            resp = f"**Today's Classes ({today_name})**\n\n"
    ...
    if any(w in lower for w in ["pyq","previous year","question paper"]):
        return (f"**PYQ Resources for {br}**\n\n"
                f"Access via **📂 PYQs** in the dashboard sidebar.\n\n"
    ...
    if any(w in lower for w in ["exam","tip","strategy","prepare","study"]):
        ...
        return (f"**Exam Prep Strategy — {br}**\n\n"
    ...
    if any(w in lower for w in ["hii","hi","hello","hey","greet"]):
        first = st.session_state.student_name.split()[0]
        return (f"Hey {first}! 👋 I'm AskMNIT, your campus AI. Ask me about:\n\n"
                "- 📊 Attendance analysis\n"
                "- 📅 Today's schedule\n"
    ...
    return (f"I'm AskMNIT — built for **{first_name}** · **{br}**.\n\n"
            "| Topic | Try asking… |\n|---|---|\n"
...
# ─────────────────────────────────────────────────────────────────────────────
# CHAT HTML COMPONENT BUILDER
# This builds the entire chat page as a self-contained HTML document.
# Python injects: messages list, student name, branch, sessions count.
# HTML sends back: user typed messages, nav actions (new chat, history, back).
# ─────────────────────────────────────────────────────────────────────────────
def build_chat_html(
    messages: list[dict],
    student_name: str,
    ...
    for i, sess in enumerate(reversed(chat_sessions)):
        ...
        history_items += f'''
        <div class="hist-item">
          <span class="hist-label">{i+1}. {lbl}</span>
          <button class="hist-load-btn" onclick="loadSession({idx})">Load</button>
        </div>'''

    if not history_items:
        history_items = '<p class="hist-empty">No saved sessions yet.<br>Click ➕ New Chat to save one.</p>'

    # Suggestion chips (hero state)
    suggestions = [
    ...
    for sug in suggestions:
        sug_safe = safe_html(sug)
        chips_html += f'<button class="chip" onclick="sendSuggestion({json.dumps(sug)})">{sug_safe}</button>'

    history_panel_display = "block" if history_open else "none"
    settings_panel_display = "block" if settings_open else "none"
    nav_hist_active = "nav-btn-active" if history_open else ""
    nav_sett_active = "nav-btn-active" if settings_open else ""
...
<style>
  ...
  *, *::before, *::after {{ box-sizing: border-box; margin: 0; padding: 0; }}
  ...
  /* ════════════════════════════════════════════════════
     TOP NAVBAR — glassmorphism fixed bar
  ════════════════════════════════════════════════════ */
  .navbar {{
    position: fixed;
  ...
  .hist-load-btn:hover {{ background: rgba(59,130,246,0.22); }}
  .hist-empty {{
    font-size: 0.77rem;
  ...
  .hero-logo {{
    ...
    box-shadow:
      ...
      0 16px 52px rgba(37,99,235,0.32),
  ...
  .hero-title {{
    ...
    line-height: 1.05;
    margin-bottom: 10px;
    animation: fadeUp 0.50s 0.06s ease both;
    text-align: center;
  }}
  .hero-title span {{
    font-weight: 300;
  ...
  .chips-row {{
    ...
    justify-content: center;
    max-width: 780px;
    animation: fadeUp 0.50s 0.14s ease both;
  }}
  .chip {{
  ...
  .input-dock {{
    ...
    padding: 12px max(20px, calc((100% - 800px)/2)) 14px;
    box-shadow: 0 -8px 40px rgba(0,0,0,0.50);
  }}

  /* The pill-shaped input container */
  .input-pill {{
    display: flex;
    align-items: center;
    gap: 0;
  ...
  .send-btn {{
    ...
    cursor: pointer;
    flex-shrink: 0;
    box-shadow: 0 3px 14px rgba(37,99,235,0.34);
  ...
  .send-btn:hover {{
    ...
    box-shadow: 0 5px 20px rgba(37,99,235,0.46);
  }}
...
<nav class="navbar">
  ...
  <div class="nav-pills">
    ...
    <button class="nav-btn {nav_hist_active}" onclick="toggleHistory()">&#9201; History</button>
    <button class="nav-btn {nav_sett_active}" onclick="toggleSettings()">&#9881; Settings</button>
    <button class="nav-back" onclick="goToDashboard()">&#128281; Dashboard</button>
  </div>
</nav>

<!-- ── DROPDOWN PANELS ── -->
<div class="dropdown-panel" id="historyPanel" style="display:{history_panel_display}">
  <div class="panel-title">Chat History</div>
  {history_items}
</div>

<div class="dropdown-panel" id="settingsPanel" style="display:{settings_panel_display}">
...
<div class="input-dock">
  <div class="input-pill" id="inputPill">
    ...
    <!-- 📎 LEFT ICON -->
    <div class="input-icon-left" title="Attach file" onclick="attachFile()">
      &#128206;
    </div>
    ...
    <textarea
      ...
      id="chatTextarea"
      placeholder="Ask anything — attendance, schedule, PYQs, fees, exams&hellip;"
      rows="1"
    ...
    <div class="input-icons-right">
      <div class="input-icon-right" title="Voice input">&#127908;</div>
      <button class="send-btn send-btn-disabled" id="sendBtn" onclick="sendMessage()" title="Send">
        &#8593;
      </button>
    </div>
...
<script>
  ...
  function postAction(type, payload) {{
    window.parent.postMessage({{ streamlit_type: type, payload: payload }}, "*");
  }}
  ...
  function toggleHistory() {{
    postAction("toggle_history", {{}});
  }}
  function toggleSettings() {{
    postAction("toggle_settings", {{}});
  }}
  function goToDashboard() {{
    postAction("go_dashboard", {{}});
  }}
  function loadSession(idx) {{
    postAction("load_session", {{idx: idx}});
  }}
  ...
  function sendMessage() {{
    ...
    sendBtn.classList.add("send-btn-disabled");
    postAction("send_message", {{text: text}});
  }}
  ...
  // ── Listen for Streamlit messages back ────────────────────────────────────
  window.addEventListener("message", function(e) {{
    if (e.data && e.data.type === "streamlit:setComponentValue") {{
      // handled by Streamlit component protocol
    }}
  }});
</script>
</body>
...
# ─────────────────────────────────────────────────────────────────────────────
# HANDLE POSTMESSAGE EVENTS FROM COMPONENT
# Streamlit components communicate via query_params or component_value.
# We use st.components.v1.html with a key and detect the component value.
# ─────────────────────────────────────────────────────────────────────────────
def process_chat_action(action_data: dict | None):
    """Process an action dict sent from the HTML component."""
    if not action_data:
        return
    ...
    elif action_type == "toggle_history":
        st.session_state.show_chat_history  = not st.session_state.show_chat_history
        st.session_state.show_chat_settings = False

    elif action_type == "toggle_settings":
        st.session_state.show_chat_settings = not st.session_state.show_chat_settings
        st.session_state.show_chat_history  = False

    elif action_type == "go_dashboard":
        st.session_state.view               = "dashboard"
        st.session_state.show_chat_history  = False
        st.session_state.show_chat_settings = False

    elif action_type == "load_session":
...
if view == "chat":
    ...
    # Hide ALL Streamlit chrome — only the iframe component will be visible
    st.markdown("""
    <style>
    ...
    /* Make the iframe fill the whole viewport */
    iframe[title="st_component_1"] {
        width: 100% !important;
        height: 100vh !important;
        border: none !important;
        display: block !important;
    }
    /* Also handle the default component title */
    ...
    # The component sends back data via Streamlit's component protocol
    # We also handle direct URL-param approach as fallback
    if component_value is not None:
        if isinstance(component_value, dict):
            process_chat_action(component_value)
            st.rerun()
...
[data-testid="stSelectbox"] label {
    color: var(--muted) !important; font-size: 0.70rem !important;
    font-weight: 600 !important; text-transform: uppercase !important; font-family: var(--sans) !important;
}
...
.settings-menu-btn .stButton > button {
    ...
    font-size: 0.82rem !important; font-weight: 600 !important; padding: 8px 16px !important; border-radius: 10px !important;
}
...
if dash_page != "My Dashboard":
    ...
    st.markdown(
        ...
        'border:1px dashed rgba(59,130,246,0.18);border-radius:16px;padding:60px 40px;text-align:center;">'
...
with h_right:
    ...
    notif_html = (
        '<div style="width:30px;height:30px;border-radius:7px;background:rgba(255,255,255,0.05);'
        'border:1px solid rgba(255,255,255,0.08);display:flex;align-items:center;justify-content:center;'
        'font-size:0.86rem;position:relative;cursor:pointer;">&#128276;'
        '<span style="position:absolute;top:-2px;right:-2px;width:7px;height:7px;border-radius:50%;'
        'background:#EF4444;border:1.5px solid #060A12;"></span></div>'
    )
    if pic_b64:
        avatar_html = (
        ...
        avatar_html = (
            '<div style="width:32px;height:32px;border-radius:50%;background:linear-gradient(135deg,#2563EB,#4F46E5);'
            'display:flex;align-items:center;justify-content:center;font-size:0.66rem;font-weight:700;color:white;'
            'font-family:\'DM Mono\',monospace;border:2px solid rgba(59,130,246,0.40);flex-shrink:0;">'
            + init_str + '</div>'
        )
...
if st.session_state.settings_mode == "profile":
    ...
    with st.expander("📅  Upload Weekly Schedule", expanded=True):
        if st.session_state.schedule_loaded:
            st.markdown('<div style="background:rgba(16,185,129,.07);border:1px solid rgba(16,185,129,.20);border-radius:9px;padding:8px 12px;margin-bottom:10px;font-size:0.74rem;color:#34D399;">&#128196; Currently loaded: <b>' + st.session_state.pdf_filename + '</b></div>', unsafe_allow_html=True)
...
if st.session_state.schedule_loaded:
    ...
    if today_slots:
        ...
        for row in rows:
            ...
            for ci,(col,slot) in enumerate(zip(cols,row)):
                ...
                is_past=(sh*60+sm)<now_hm; tc=TYPE_COLORS.get(slot["type"],"#60A5FA")
    ...
    st.markdown('<div style="background:rgba(59,130,246,.04);border:1px dashed rgba(59,130,246,.20);border-radius:9px;padding:9px 13px;margin-bottom:12px;font-size:0.73rem;color:rgba(148,163,184,.48);">&#128196;  Use <b>&#9881;&#65039; Menu &#8594; Upload Weekly Schedule</b> to activate the auto-planner.</div>', unsafe_allow_html=True)
