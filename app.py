...
if view == "chat":
    ...
    # ── File picker query param ───────────────────────────────────────────
    fpname = st.query_params.get("_fp", "")
    if fpname and fpname != st.session_state.attached_file_name:
        ...
        try: del st.query_params["_fp"]
        except: pass
        st.rerun()

    # ── Sidebar action handler ────────────────────────────────────────────
    _sba = st.query_params.get("_sba", "")
    if _sba:
        try: del st.query_params["_sba"]
        except: pass
        if _sba == "new_chat":
...
function goAction(action){{
  ...
  url.searchParams.set('_sba', action);
  window.parent.location.href = url.toString();
}}
function openERP(){{
  window.parent.open('https://erp.mnit.ac.in','_blank');
}}
function loadSession(idx){{
  // future: load session by idx
  closeSb();
}}
...
function handleFile(input){{
  ...
  var url = new URL(window.parent.location.href);
  url.searchParams.set('_fp', fname);
  window.parent.location.href = url.toString();
}}
function clearFile(){{
  var url = new URL(window.parent.location.href);
  url.searchParams.delete('_fp');
  window.parent.location.href = url.toString();
}}
...
function sendMsg(){{
  ...
  window.parent.postMessage({{type:'askmnitSend', text:txt}}, '*');
}}
...
function startMic(){{
  navigator.mediaDevices.getUserMedia({{audio:true}}).then(function(stream){{
    ...
    mediaRecorder.onstop=function(){{
      ...
      window.parent.postMessage({{type:'askmnitVoiceDone'}},'*');
    }};
...
// ── Listen for send events from parent (postMessage back) ─────────
window.addEventListener('message', function(e){{
  if(!e.data) return;
  if(e.data.type==='askmnitSend'){{
    var url = new URL(window.parent.location.href);
    url.searchParams.set('_msg', encodeURIComponent(e.data.text));
    window.parent.location.href = url.toString();
  }}
}});
</script>
...
""", height=st.session_state.get("_chat_height", 720) if has_messages else 680, scrolling=False)

    # ── Handle postMessage send via query param ───────────────────────────
    _msg_raw = st.query_params.get("_msg", "")
    if _msg_raw:
        import urllib.parse
        _msg_text = urllib.parse.unquote(_msg_raw).strip()
