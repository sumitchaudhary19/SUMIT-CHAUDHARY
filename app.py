...
# ║  v4.1 — Left Vertical Nav Panel (no sidebar in chat view)                   ║
# ╚══════════════════════════════════════════════════════════════════════════════╝
...
_DEFAULTS: dict = {
    ...
    "voice_transcript":   "",
    "_voice_submit":      False,
    # LEFT PANEL STATE — None | "history" | "settings" | "theme"
    "left_panel":         None,
}
...
# ─────────────────────────────────────────────────────────────────────────────
# AI RESPONSE GENERATOR  (original v4.0 logic, unchanged)
# ─────────────────────────────────────────────────────────────────────────────
def generate_ai_response(last: str) -> str:
    ...
    if "voice" in lower or "recorded" in lower or "[voice" in lower:
        fn = st.session_state.student_name.split()[0]
        return fmt(f"🎤 Voice message received, {fn}! For best results, try asking about attendance, schedule, PYQs, fees, or exam strategy.")
...
def get_theme_css() -> str:
    ...
    if is_light:
        return """
:root{--bg:#F0F4FF;--surf:#FFFFFF;--surf2:#E8EEFF;--bar-bg:#FFFFFF;
  --border:rgba(0,0,0,0.10);--border2:rgba(0,0,0,0.18);--text:#1E2A3A;
  --muted:rgba(60,80,110,0.60);--accent:#2563EB;}
html,body,[data-testid="stApp"],[data-testid="stAppViewContainer"]{background:#F0F4FF !important;color:#1E2A3A !important;}
.gemini-bar [data-testid="stForm"]{background:#FFFFFF !important;border-color:rgba(37,99,235,0.22) !important;box-shadow:0 4px 24px rgba(37,99,235,0.10) !important;}
...
[data-testid="stChatMessage"]:has([data-testid="chatAvatarIcon-user"]){background:rgba(37,99,235,0.07) !important;border-color:rgba(37,99,235,0.15) !important;}
.chat-left-rail{background:rgba(240,244,255,0.98) !important;border-right-color:rgba(37,99,235,0.14) !important;}
.chat-drawer{background:rgba(235,241,255,0.99) !important;border-right-color:rgba(37,99,235,0.13) !important;}
.rail-icon-btn{color:rgba(60,80,110,0.55) !important;}
.rail-icon-btn:hover{background:rgba(37,99,235,0.10) !important;border-color:rgba(37,99,235,0.28) !important;color:#1E3A8A !important;}
.rail-icon-btn.rail-active{background:rgba(37,99,235,0.15) !important;border-color:rgba(37,99,235,0.40) !important;color:#1D4ED8 !important;}
.drawer-header{color:rgba(60,80,110,0.40) !important;border-bottom-color:rgba(0,0,0,0.06) !important;}
.drawer-history-item{color:rgba(60,80,110,0.62) !important;}
.drawer-history-item:hover{background:rgba(37,99,235,0.08) !important;color:#1E3A8A !important;}
"""
    return """
:root{--bg:#070B14;--surf:#0B1120;--surf2:#0E1726;--bar-bg:#1A1E2E;
  --border:rgba(255,255,255,0.08);--border2:rgba(255,255,255,0.14);
  --text:#E2E8F0;--muted:rgba(148,163,184,0.55);--accent:#3B82F6;}
...
/* ════════════════════════════════════════════════════════
   HIDE Streamlit sidebar in chat view
   ════════════════════════════════════════════════════════ */
.hide-sb [data-testid="stSidebar"],
.hide-sb [data-testid="stSidebarCollapseButton"],
.hide-sb [data-testid="collapsedControl"]{display:none !important;}

/* ════════════════════════════════════════════════════════
   LEFT ICON RAIL  — 58px fixed strip
   ════════════════════════════════════════════════════════ */
.chat-left-rail{
  position:fixed;top:0;left:0;bottom:0;width:58px;
  background:rgba(9,13,26,0.97);
  border-right:1px solid rgba(59,130,246,0.14);
  backdrop-filter:blur(20px) saturate(180%);
  z-index:1000;
  display:flex;flex-direction:column;align-items:center;
  padding:14px 0 14px;gap:4px;
}
.rail-logo{
  width:34px;height:34px;border-radius:9px;
  background:linear-gradient(135deg,#2563EB,#4F46E5);
  display:flex;align-items:center;justify-content:center;
  font-size:0.90rem;font-weight:700;color:white;
  box-shadow:0 3px 12px rgba(37,99,235,0.30);
  margin-bottom:14px;flex-shrink:0;
}
.rail-divider{width:30px;height:1px;background:rgba(255,255,255,0.07);margin:5px 0;flex-shrink:0;}
.rail-spacer{flex:1;}
...
.rail-icon-btn{
  ...
  color:rgba(148,163,184,0.50);font-size:1.05rem;
...
.rail-icon-btn.rail-danger{color:rgba(252,165,165,0.50);}
.rail-icon-btn.rail-danger:hover{background:rgba(239,68,68,0.14);border-color:rgba(239,68,68,0.30);color:#FCA5A5;}

/* Tooltip on hover */
.rail-icon-btn::after{
  content:attr(data-tip);
  position:absolute;left:52px;top:50%;transform:translateY(-50%);
  background:#0D1828;color:#E2E8F0;
  ...
  pointer-events:none;opacity:0;transition:opacity 0.15s;z-index:2100;
}
.rail-icon-btn:hover::after{opacity:1;}

/* ════════════════════════════════════════════════════════
   DRAWER  — slides out from left:58px
   ════════════════════════════════════════════════════════ */
.chat-drawer{
  position:fixed;top:0;left:58px;bottom:0;width:244px;
  background:rgba(11,17,32,0.98);
  border-right:1px solid rgba(59,130,246,0.12);
  backdrop-filter:blur(24px) saturate(180%);
  z-index:999;padding:20px 14px 18px;overflow-y:auto;
  animation:drawerSlideIn 0.22s cubic-bezier(0.22,0.61,0.36,1) both;
}
...
.drawer-header{
  ...
  color:rgba(148,163,184,0.38);text-transform:uppercase;letter-spacing:1.4px;
...
.drawer-history-item{
  ...
  font-size:0.78rem;color:rgba(148,163,184,0.60);margin-bottom:2px;
}
.drawer-history-item:hover{background:rgba(59,130,246,0.09);color:#BAE6FD;}
.drawer-history-dot{width:5px;height:5px;border-radius:50%;background:#3B82F6;flex-shrink:0;}
.drawer-day-label{
  font-size:0.57rem;color:rgba(148,163,184,0.30);
  text-transform:uppercase;letter-spacing:0.7px;
  margin:8px 0 3px 10px;
}

/* Drawer widgets area (fixed position overlay matching drawer) */
.drawer-widgets{
  position:fixed;top:52px;left:60px;width:240px;
  z-index:1000;padding:4px 14px 18px;
  overflow-y:auto;max-height:calc(100vh - 56px);
}
/* Drawer selectbox override */
.drawer-widgets [data-testid="stSelectbox"]>div>div{background:rgba(255,255,255,0.05) !important;border:1px solid rgba(255,255,255,0.12) !important;border-radius:8px !important;color:#E2E8F0 !important;font-size:0.82rem !important;}
.drawer-widgets [data-testid="stSelectbox"] label{color:rgba(148,163,184,0.50) !important;font-size:0.67rem !important;font-weight:600 !important;text-transform:uppercase !important;}
/* Drawer toggle */
.drawer-widgets [data-testid="stToggle"] label{color:rgba(226,232,240,0.78) !important;font-size:0.83rem !important;}
/* Drawer buttons */
.drawer-widgets .stButton>button{background:rgba(255,255,255,0.05) !important;border:1px solid rgba(255,255,255,0.12) !important;border-radius:8px !important;color:rgba(226,232,240,0.70) !important;box-shadow:none !important;font-size:0.80rem !important;font-weight:600 !important;padding:7px 12px !important;}
.drawer-widgets .stButton>button:hover{background:rgba(59,130,246,0.14) !important;color:#BAE6FD !important;border-color:rgba(59,130,246,0.30) !important;transform:none !important;}
.drawer-danger-btn .stButton>button{background:rgba(239,68,68,0.09) !important;border:1px solid rgba(239,68,68,0.22) !important;color:#FCA5A5 !important;}
.drawer-danger-btn .stButton>button:hover{background:rgba(239,68,68,0.18) !important;}

/* ════════════════════════════════════════════════════════
   MAIN CONTENT — padded right of rail
   ════════════════════════════════════════════════════════ */
.chat-body-offset{
  padding-left:58px !important;
  transition:padding-left 0.22s cubic-bezier(0.22,0.61,0.36,1);
}
.chat-body-offset.drawer-open{padding-left:302px !important;}
...
.gemini-attach .stButton>button{background:transparent !important;border:none !important;border-radius:50% !important;color:rgba(148,163,184,0.55) !important;font-size:1.2rem !important;width:42px !important;height:42px !important;min-width:42px !important;padding:0 !important;box-shadow:none !important;transition:background 0.16s,color 0.16s !important;}
...
.gemini-mic .stButton>button:hover{background:rgba(255,255,255,0.09) !important;color:rgba(186,230,253,0.80) !important;transform:none !important;opacity:1 !important;}
...
/* Voice banner */
.listening-banner{display:flex;align-items:center;gap:8px;margin:10px auto 0;padding:8px 18px;background:rgba(239,68,68,0.09);border:1px solid rgba(239,68,68,0.22);border-radius:10px;font-size:0.80rem;color:#FCA5A5;max-width:800px;animation:fadeInUp 0.25s ease both;}
...
/* Attach panel */
.attach-panel{max-width:800px;margin:10px auto 4px;padding:14px 16px;background:rgba(59,130,246,0.05);border:1px dashed rgba(59,130,246,0.28);border-radius:14px;animation:fadeInUp 0.2s ease both;}

/* Fixed bottom bar — left edge tracks rail/drawer */
.gemini-bar-anchored{position:fixed;bottom:0;right:0;z-index:900;background:rgba(7,11,20,0.97);backdrop-filter:blur(24px) saturate(160%);border-top:1px solid rgba(59,130,246,0.12);padding:10px max(16px,calc((100% - 860px)/2)) 12px;}
...
.sb-history-item{display:flex;align-items:center;gap:8px;padding:7px 16px;cursor:pointer;border-radius:0;transition:background 0.14s;font-size:0.80rem;color:rgba(148,163,184,0.70);border-bottom:1px solid rgba(255,255,255,0.03);}
...
[data-testid="stSidebar"] [data-testid="stSelectbox"]>div>div{background:rgba(255,255,255,0.05) !important;border:1px solid rgba(255,255,255,0.12) !important;border-radius:8px !important;color:#E2E8F0 !important;font-size:0.83rem !important;}
[data-testid="stSidebar"] [data-testid="stSelectbox"] label{color:rgba(148,163,184,0.65) !important;font-size:0.70rem !important;font-weight:600 !important;text-transform:uppercase !important;letter-spacing:0.5px !important;}
...
.sb-theme-dark-btn .stButton>button:hover{border-color:rgba(59,130,246,0.65) !important;transform:none !important;}
...
.sb-theme-light-btn .stButton>button:hover{background:rgba(240,244,255,0.25) !important;transform:none !important;}
...
.nav-pill .stButton>button{background:rgba(255,255,255,0.06) !important;border:1px solid rgba(255,255,255,0.10) !important;border-radius:999px !important;color:rgba(226,232,240,0.72) !important;font-size:0.78rem !important;font-weight:500 !important;padding:6px 15px !important;box-shadow:none !important;}
.nav-pill .stButton>button:hover{background:rgba(59,130,246,0.16) !important;border-color:rgba(59,130,246,0.38) !important;color:#BAE6FD !important;transform:none !important;}
.nav-back .stButton>button{background:rgba(255,255,255,0.06) !important;border:1px solid rgba(255,255,255,0.12) !important;border-radius:10px !important;color:rgba(226,232,240,0.65) !important;font-size:0.78rem !important;font-weight:600 !important;padding:6px 14px !important;box-shadow:none !important;}
.nav-back .stButton>button:hover{background:rgba(59,130,246,0.14) !important;color:#BAE6FD !important;border-color:rgba(59,130,246,0.30) !important;}
...
[data-testid="stPopover"]>div{background:#0D1828 !important;border:1px solid rgba(59,130,246,0.30) !important;border-radius:16px !important;box-shadow:0 16px 56px rgba(0,0,0,0.65) !important;}
button[data-testid="stPopoverButton"]{background:rgba(255,255,255,0.06) !important;border:1px solid rgba(255,255,255,0.10) !important;border-radius:999px !important;color:rgba(226,232,240,0.72) !important;font-size:0.78rem !important;font-weight:500 !important;padding:6px 15px !important;box-shadow:none !important;}
button[data-testid="stPopoverButton"]:hover{background:rgba(59,130,246,0.16) !important;border-color:rgba(59,130,246,0.38) !important;color:#BAE6FD !important;}
...
# ═════════════════════════════════════════════════════════════════════════════
# NATIVE FILE PICKER  (original v4.0 — unchanged)
# ═════════════════════════════════════════════════════════════════════════════
...
# ═════════════════════════════════════════════════════════════════════════════
# VOICE RECORDER  (original v4.0 — unchanged)
# ═════════════════════════════════════════════════════════════════════════════
...
<script>
(function() {{
  var mediaRecorder = null; var audioChunks = []; var isRecording = false;
...
# ═════════════════════════════════════════════════════════════════════════════
# GEMINI BAR  (original v4.0 — unchanged)
# ═════════════════════════════════════════════════════════════════════════════
def render_gemini_bar(bar_key: str, hero_mode: bool = True):
    ...
    if st.session_state.attached_file_name:
        ...
        chip_html = (f'<div class="file-chip" title="{fname}"><span class="file-chip-icon">📎</span>{short}</div>')

    st.markdown(f'<div class="gemini-bar" style="max-width:800px;margin:0 auto;{anim_style}">', unsafe_allow_html=True)

    if chip_html:
        st.markdown(f'<div style="display:flex;align-items:center;padding:0 16px 6px;">{chip_html}<button onclick="clearChip_{bar_key}()" style="background:none;border:none;color:rgba(148,163,184,0.5);cursor:pointer;font-size:0.75rem;margin-left:4px;padding:0 4px;" title="Remove file">✕</button></div>', unsafe_allow_html=True)
    ...
    with st.form(key=f"gemini_form_{bar_key}", clear_on_submit=True):
        ...
        with c_input:
            ...
            user_text = st.text_input(label="__gi__", placeholder=placeholder, key=f"gi_text_{bar_key}", label_visibility="collapsed")
        with c_mic:
    ...
    st.markdown(f'<script>function clearChip_{bar_key}(){{var url=new URL(window.location.href);url.searchParams.delete("fp_{bar_key}");window.location.href=url.toString();}}</script>', unsafe_allow_html=True)
    st.markdown('</div>', unsafe_allow_html=True)
...
#  Replaces the old sidebar + top navbar in chat view.
#  Nav items (top to bottom):
#    Logo → ⏱ History → ⚙ Settings → 🌗 Theme → divider → ✚ New Chat → 🏠 Dashboard → [spacer] → 🚪 Logout
#  Clicking icon toggles its drawer panel.
...
def render_drawer():
    """Renders the sliding drawer panel when a panel is open."""
    p = st.session_state.left_panel
    if p is None:
        return

    titles = {"history": "⏱ Chat History", "settings": "⚙️ Settings", "theme": "🌗 Theme"}
    # Background drawer shell (HTML fixed div)
    st.markdown(f'<div class="chat-drawer"><div class="drawer-header">{titles.get(p,"")}</div></div>', unsafe_allow_html=True)

    # Streamlit widgets rendered in an overlay container positioned over the drawer
    st.markdown('<div class="drawer-widgets">', unsafe_allow_html=True)

    if p == "history":
        # Live sessions
        if st.session_state.chat_sessions:
            st.markdown('<div style="font-size:0.60rem;color:rgba(59,130,246,0.55);text-transform:uppercase;letter-spacing:0.6px;margin-bottom:6px;padding-left:4px;">This session</div>', unsafe_allow_html=True)
            for i, sess in enumerate(reversed(st.session_state.chat_sessions[-5:])):
                label = sess["label"][:26] + ("..." if len(sess["label"]) > 26 else "")
                c1, c2 = st.columns([5, 1])
                with c1:
                    st.markdown(f'<div class="drawer-history-item"><div class="drawer-history-dot"></div>{label}</div>', unsafe_allow_html=True)
                with c2:
                    st.markdown('<div class="ghost-btn">', unsafe_allow_html=True)
                    if st.button("↩", key=f"dr_load_{i}"):
        ...
        for day_lbl, title in SIDEBAR_HISTORY:
            if day_lbl != current_day:
                current_day = day_lbl
                st.markdown(f'<div class="drawer-day-label">{day_lbl}</div>', unsafe_allow_html=True)
            st.markdown(f'<div class="drawer-history-item"><div class="drawer-history-dot" style="background:rgba(148,163,184,0.28);"></div>{title}</div>', unsafe_allow_html=True)
        ...
        new_style = st.selectbox(
            ...
            key="dr_response_style",
        )
        if new_style != st.session_state.response_style:
            st.session_state.response_style = new_style; st.rerun()

        st.session_state.strict_mode  = st.toggle("Strict Academic Mode", value=st.session_state.strict_mode,  key="dr_strict")
        st.session_state.voice_output = st.toggle("Voice Output",         value=st.session_state.voice_output, key="dr_voice")

        st.markdown("<div style='height:10px'></div>", unsafe_allow_html=True)
        st.markdown('<div class="drawer-danger-btn">', unsafe_allow_html=True)
        if st.button("🗑 Clear All Chats", key="dr_clear", use_container_width=True):
            st.session_state.chat_messages      = []
            st.session_state.chat_sessions      = []
            st.session_state.show_uploader      = False
            st.session_state.attached_file_name = ""
            st.session_state.left_panel         = None
            st.toast("All chats cleared.", icon="🗑"); st.rerun()
        st.markdown('</div>', unsafe_allow_html=True)

    elif p == "theme":
        t1, t2 = st.columns(2)
        with t1:
            active = st.session_state.chat_theme == "dark"
            label  = "🌑 Dark" if active else "Dark"
            st.markdown('<div class="sb-theme-dark-btn">', unsafe_allow_html=True)
            if st.button(label, key="dr_dark", use_container_width=True):
                st.session_state.chat_theme = "dark"; st.rerun()
            st.markdown('</div>', unsafe_allow_html=True)
        with t2:
            active = st.session_state.chat_theme == "light"
            label  = "☀️ Light" if active else "Light"
            st.markdown('<div class="sb-theme-light-btn">', unsafe_allow_html=True)
            if st.button(label, key="dr_light", use_container_width=True):
                st.session_state.chat_theme = "light"; st.rerun()
            st.markdown('</div>', unsafe_allow_html=True)

        st.markdown(f'<div style="margin-top:16px;font-size:0.66rem;color:rgba(148,163,184,0.34);font-family:\'DM Mono\',monospace;line-height:1.7;">{st.session_state.student_name}<br>{st.session_state.branch} · {st.session_state.semester}</div>', unsafe_allow_html=True)

    st.markdown('</div>', unsafe_allow_html=True)  # close .drawer-widgets
...
if view == "chat":
    ...
    # ── 4. Voice / file query-param processing (original v4.0 logic) ─────────
    ...
    if st.session_state._voice_submit:
        ...
        st.toast("Voice message sent!", icon="🎤"); st.rerun()
    ...
    if not has_messages:
        ...
        if hero_action is not None:
            act = hero_action[0]
            if act == "send":
                dispatch_message(hero_action[1]); st.session_state.attached_file_name = ""; st.rerun()
            elif act == "mic":
                if hero_action[1] == "start":
                    st.session_state.is_recording = True
                    st.markdown("""<script>(function poll(){var f=document.querySelector('iframe');if(f&&f.contentWindow){f.contentWindow.postMessage({type:'startRecording_hero'},'*');}else{setTimeout(poll,200);}})();</script>""", unsafe_allow_html=True)
                    st.toast("🎤 Recording started — speak now!", icon="🎤")
                else:
                    st.session_state.is_recording = False
                    st.markdown("""<script>(function(){var frames=document.querySelectorAll('iframe');frames.forEach(function(f){if(f.contentWindow)f.contentWindow.postMessage({type:'stopRecording_hero'},'*');});})();</script>""", unsafe_allow_html=True)
                    st.toast("⏹ Processing voice...", icon="⏳")
                st.rerun()
            elif act == "attach":
                st.markdown("""<script>setTimeout(function(){var frames=document.querySelectorAll('iframe');frames.forEach(function(f){if(f.contentWindow)f.contentWindow.postMessage({type:'triggerPicker_hero'},'*');});},100);</script>""", unsafe_allow_html=True)
    ...
    # ── ACTIVE CHAT STATE ─────────────────────────────────────────────────────
        ...
        if anchored_action is not None:
            ...
            if act == "send":
                dispatch_message(anchored_action[1]); st.session_state.attached_file_name = ""; st.rerun()
            elif act == "mic":
                if anchored_action[1] == "start":
                    st.session_state.is_recording = True
                    st.markdown("""<script>setTimeout(function(){var frames=document.querySelectorAll('iframe');frames.forEach(function(f){if(f.contentWindow)f.contentWindow.postMessage({type:'startRecording_anchored'},'*');});},150);</script>""", unsafe_allow_html=True)
                    st.toast("🎤 Recording started — speak now!", icon="🎤")
                else:
                    st.session_state.is_recording = False
                    st.markdown("""<script>setTimeout(function(){var frames=document.querySelectorAll('iframe');frames.forEach(function(f){if(f.contentWindow)f.contentWindow.postMessage({type:'stopRecording_anchored'},'*');});},50);</script>""", unsafe_allow_html=True)
                    st.toast("⏹ Processing voice...", icon="⏳")
                st.rerun()
            elif act == "attach":
                st.markdown("""<script>setTimeout(function(){var frames=document.querySelectorAll('iframe');frames.forEach(function(f){if(f.contentWindow)f.contentWindow.postMessage({type:'triggerPicker_anchored'},'*');});},100);</script>""", unsafe_allow_html=True)
                st.session_state.show_uploader = True; st.rerun()
...
with h_right:
    ...
    av_html = (f'<img src="{pp}" style="width:36px;height:36px;border-radius:50%;object-fit:cover;border:2px solid {bh}55;">' if pp else f'<div style="width:36px;height:36px;border-radius:50%;background:linear-gradient(135deg,{bh},{bh}88);display:flex;align-items:center;justify-content:center;font-size:0.85rem;font-weight:700;color:#fff;border:2px solid {bh}55;">{initials(nm)}</div>')
